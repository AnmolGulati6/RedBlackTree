...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
  ...
  public boolean insert(T data) throws NullPointerException, IllegalArgumentException {
    ...
    newNode.blackHeight = 0; // all new nodes are red by default
    if (root == null) {
      root = newNode;
      size++;
  ...
  /**
   * Recursive method that makes sure that the red black tree doesn't make any violations after a
   * new node is inserted
   * 
   * @param newly inserted redNode of type Node<T>
   */
  protected void enforceRBTreePropertiesAfterInsert(Node<T> redNode) {
    Node<T> parent;
    Node<T> grandParent;
    while (redNode != root && redNode.blackHeight != 1 && redNode.parent.blackHeight == 0) {
      //RECURSIVE CASE 
      parent = redNode.parent;
      grandParent = redNode.parent.parent;
      ...
      if (root.blackHeight == 1) { //checks violation that root is black, not red
        root.blackHeight = 0;
      }
    }
  }
  ...
  /**
   * helper method for right rotation, when right child of parent is being called to rotate, this
   * method is called This method performs a right rotation
   * 
   * @param Node<T> child is the child we are rotating
   * @param Node<T> is the parent of the node we are rotating
   */
  ...
  private void rightRotation(Node<T> child, Node<T> parent) {
    ...
    if (parent.parent == null) {
      this.root = child;
    } else if (parent.equals(parent.parent.leftChild)) {
  ...
  /**
   * helper method for left rotation, when left child of parent is being called to rotate, this
   * method is called This method performs a left rotation
   * 
   * @param Node<T> child is the child we are rotating
   * @param Node<T> is the parent of the node we are rotating
   */
  ...
  private void leftRotation(Node<T> child, Node<T> parent) {
    ...
    if (parent.parent == null) {
      this.root = child;
    } else if (parent.equals(parent.parent.rightChild)) {
      parent.parent.rightChild = child;
    } else {
      parent.parent.leftChild = child;
    }
  ...
  /**
   * Tester method which creates a redblacktree and checks if it's root is null as intended
   * 
   * @return true if root is null as intended when initially declared, false otherwise
   */
  ...
  /**
   * Tester method for right rotation initializes a valid binary search tree and calls rotate on the
   * leftChild, and it's parent try and catch method to check for valid output and any bugs
   * 
   * @return true if rotation is correct, false if any bugs or wrong output
   */
  ...
  public static boolean test2() {
    ...
    try {
      RedBlackTree<Integer> obj = new RedBlackTree<Integer>();
      String correctOutput = "[ 2, 1, 4, 3, 5 ]";

      obj.root = new Node<Integer>(4);
      obj.root.leftChild = new Node<Integer>(2);
      obj.root.rightChild = new Node<Integer>(5);
      obj.root.leftChild.leftChild = new Node<Integer>(1);
      obj.root.leftChild.rightChild = new Node<Integer>(3);
      obj.rotate(obj.root.leftChild, obj.root);
      if (!(obj.toLevelOrderString()).equals(correctOutput)) {
        return false;
      }
  ...
  /**
   * Tester method for left rotation initializes a valid binary search tree and calls rotate on the
   * rightChild, and it's parent try and catch method to check for valid output and any bugs
   * 
   * @return true if rotation is correct, false if any bugs or wrong output
   */
  ...
  public static boolean test3() {
    ...
    try {
      RedBlackTree<Integer> obj = new RedBlackTree<Integer>();
      String correctOutput = "[ 4, 2, 5, 1, 3 ]";

      obj.root = new Node<Integer>(2);
      obj.root.leftChild = new Node<Integer>(1);
      obj.root.rightChild = new Node<Integer>(4);
      obj.root.rightChild.rightChild = new Node<Integer>(5);
      obj.root.rightChild.leftChild = new Node<Integer>(3);

      obj.rotate(obj.root.rightChild, obj.root);
      if (!(obj.toLevelOrderString()).equals(correctOutput)) {
        return false;
      }
  ...
  public static String test4() {
    ...
    return testing.toInOrderString();
